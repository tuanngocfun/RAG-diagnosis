...
def _summarize_diagnosis_types(cases: List[Dict]) -> Dict[str, int]:
...
def _log_runtime_device_visibility() -> None:
    """Print CUDA visibility to catch accidental CPU fallback early."""
    ...
    try:
        ...
        print(
            f"CUDA visibility: CUDA_VISIBLE_DEVICES={cvd}, "
            f"torch.cuda.is_available={cuda_available}, device_count={cuda_count}"
        )
        if cvd not in {"", "ALL", None} and not cuda_available:
            print(
                "Warning: no CUDA device visible with current CUDA_VISIBLE_DEVICES; "
                "embedding/indexing will run on CPU and be much slower."
            )
    except Exception as exc:
        print(f"CUDA visibility check unavailable: {exc}")
...
class QdrantIndexer:
    ...
    def _upsert_points_batched(
        ...
        """Upsert points to Qdrant in small batches with retry/backoff."""
        if not points:
            return 0
        ...
        print(
            f"Upsert plan for {collection_name}: "
            f"points={len(points)}, batch_size={batch_size}, batches={n_batches}"
        )
        for i in tqdm(range(0, len(points), batch_size), desc=f"Upserting {collection_name}"):
            ...
            for attempt in range(max_retries):
                try:
                    ...
                    print(
                        f"Upsert retry {attempt + 1}/{max_retries} "
                        f"for batch {(i // batch_size) + 1}: {e}; sleeping {sleep_s}s"
                    )
                    time.sleep(sleep_s)
    ...
    def index_chunks_e5(
        ...
        # Upsert to Qdrant in small batches to avoid network/write timeouts
        self._upsert_points_batched(collection_name=collection_name, points=points, batch_size=64)
    ...
    def index_captions_biomedclip(
        ...
        if not captions:
            ...
            print(
                f"Caption coverage: 0/{total_image_entries} image entries have non-empty captions. "
                "Lane 2 caption collection will remain empty."
            )
            return 0

        print(
            f"Caption coverage: {len(captions)}/{total_image_entries} image entries have non-empty captions"
        )
        ...
        # Encode in batches
        points = []
        caption_batches = (len(captions) + batch_size - 1) // batch_size
        print(
            f"Caption encoding plan: captions={len(captions)}, "
            f"batch_size={batch_size}, batches={caption_batches}"
        )
    ...
    def index_images_biomedclip(
        ...
        # Encode in batches
        points = []
        image_batches = (len(images) + batch_size - 1) // batch_size
        print(
            f"Image encoding plan: images={len(images)}, "
            f"batch_size={batch_size}, batches={image_batches}"
        )
        ...
        if points:
            self._upsert_points_batched(collection_name=collection_name, points=points, batch_size=32)
...
def index_train_data(
    ...
    print("=" * 60)
    _log_runtime_device_visibility()
    print(f"\nLoading train cases from {TRAIN_JSONL}...")
    with open(TRAIN_JSONL) as f:
        cases = [json.loads(line) for line in f]
    print(f"Loaded {len(cases)} train cases")
    diagnosis_type_counts = _summarize_diagnosis_types(cases)
    print(f"Non-Leishmaniasis cases: {diagnosis_type_counts.get('Non-Leishmaniasis', 0)}")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Index train corpus into Qdrant and BM25")
    parser.add_argument(
        "--strategy",
        default="fixed",
    ...
    parser.add_argument(
        ...
        dest="index_images",
        action="store_true",
        help="Index BiomedCLIP image vectors for multimodal retrieval",
    )
    parser.add_argument(
        "--no-index-images",
        dest="index_images",
        action="store_false",
        help="Skip BiomedCLIP image vector indexing",
    )
    parser.set_defaults(index_images=True)
    args = parser.parse_args()

    if not args.index_images:
        print("WARNING: Image indexing is disabled; Lane 2 image retrieval will have 0 vectors.")

    stats = index_train_data(strategy=args.strategy, index_images=args.index_images)
    print("\n" + "="*50)
