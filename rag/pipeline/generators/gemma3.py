...
class Gemma3Generator:
    ...
    def __init__(
        ...
        Args:
            ...
            random_seed: Random seed for stochastic decoding reproducibility
            prompt_mode: Prompt template mode (STRICT_CONTEXT, BALANCED, NO_CONTEXT)
            model_path: Optional explicit HuggingFace model ID override
        """
        ...
        # Seed control for reproducible stochastic generation (temperature > 0)
        ...
        # Keep metadata for logging without passing unsupported kwargs to generate()
        self.decoding_params["random_seed"] = random_seed
    ...
    def generate(
        ...
        try:
            if self.use_vision and active_query_images:
                ...
                images = self._load_images(active_query_images)
                if images:
            ...
            # Robust fallback for GPU memory spikes on image-heavy samples
            err_msg = str(e).lower()
            is_oom = "outofmemory" in err_msg or "out of memory" in err_msg or "cuda oom" in err_msg
            if is_oom and active_query_images:
                try:
                    if self._torch.cuda.is_available():
                        self._torch.cuda.empty_cache()
                    fallback_prompt = build_rag_prompt(
                        query,
                        contexts,
                        query_images=None,
                        context_images=context_images if context_images else None,
                        prompt_mode=self.prompt_mode,
                    ) if use_rag_prompt else query
                    inputs = self.tokenizer(
                        fallback_prompt,
                        return_tensors="pt",
                        truncation=True,
                    ...
                    return f"[Generation Error: {type(fallback_error).__name__}: {fallback_error}]"
...
def get_gemma3_generator(
    ...
    return Gemma3Generator(variant=variant, use_vision=use_vision, model_path=model_path)
