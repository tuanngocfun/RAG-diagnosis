...
LEISH_TYPE_OPTIONS = "|".join(SUPPORTED_LEISH_TYPES)
LEISH_TYPE_QUESTION_LIST = ", ".join(SUPPORTED_LEISH_TYPES[:-1])
...
{{
  "diagnosis": "<single specific diagnosis>",
    "leish_type": "<{leish_type_options}>",
  "confidence": "<high|medium|low>",
    "ranked_differential": [
        "<most likely differential #1>",
        "<differential #2>",
        "<differential #3>"
    ],
...
""".format(leish_type_options=LEISH_TYPE_OPTIONS)
...
DIAGNOSIS_QUESTION_WITH_TYPE = (
    "Provide a ranked top-3 differential diagnosis for this patient. "
    "Clearly mark Rank 1 as the most likely diagnosis. "
    f"If any rank is leishmaniasis, specify the clinical form ({LEISH_TYPE_QUESTION_LIST}). "
    "If evidence favors non-leish etiologies, include Non-Leishmaniasis in the ranked list and mark it Rank 1 when appropriate."
)
...
LEAK_PATTERNS = [
    ...
    r"leishmaniasis",
    r"leishmanial",
    # Standalone abbreviations (word-boundary guarded to avoid false positives)
    r"\b(?:CL|VL|MCL|DCL|LCL|DsCL)\b",
]
