...
class CFG:
    ...
    RERANKER_MODEL_ID: str = "ncbi/MedCPT-Query-Encoder"
...
class TextCrossReranker:
    """
    ncbi/MedCPT-Query-Encoder cross-encoder for medical query-document pairs.
    ...
    def score(self, query: str, docs: List[str], max_length: int = 512) -> List[float]:
        ...
        out = self.model(**enc)
        # For MedCPT, apply softmax for medical relevance probabilities
        logits = out.logits
        if logits.ndim == 2 and logits.shape[1] == 1:
            # Binary classification case - apply sigmoid
            ...
            # Handle other cases
            scores = torch.sigmoid(logits.squeeze() if logits.ndim > 1 else logits)
