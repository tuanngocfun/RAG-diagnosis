...
from rag.test.gemini25pro_qdrant_bge import (
    ...
    _qdrant_search, extractive_spans, find_case_pdf
)
...
def select_images(seed: List[str], hits: List[Dict[str, Any]], take: int) -> List[str]:
    """
    Return up to `take` image paths:
    1) use valid seed paths
    2) use repaired hit paths
    3) FINAL FALLBACK: first 2–3 pages from the same doc_id (already-rendered PNGs)
    """
    out, seen = [], set()
    ...
    if fallback_doc:
        ...
        if case_dir:
            ...
            # If still short, try conventional filenames directly
            i = 0
            while len(out) < take and i < 3:
                p = case_dir / "pages" / f"page_{i+1:04d}.png"
                if p.exists():
                    sp = str(p)
                    if sp not in seen:
                        out.append(sp); seen.add(sp)
...
class BatchProcessor:
    ...
    def enhanced_qdrant_ask_text(self, question: str, doc_id: Optional[str] = None, 
        ...
        """
        Dual-vector retrieval (text + cross-modal text->image) with clinical boosts.
        Merges & dedupes hits, then applies reranker and your early-page preference.
        """
        qv_text = self.encoder.embed_texts([question])[0]

        # Detect clinical questions (boost recall & early pages)
        ...
        if clinical_question and doc_id and len(raw_items) < max(1, top_k // 2):
            ...
            seen = {(r["doc_id"], r["page_index"]) for r in raw_items}
        ...
        # --- Rerank (give a bit more weight for clinical Qs)
    ...
    def answer_with_images(self, question: str, image_paths: List[str], 
        ...
        other_hits = [h for h in hits[:12] if h.get("page_index", 999) > 2]
        ...
        for h in early_hits + other_hits:
            excerpt = (h.get("text_excerpt") or "").strip()
            if excerpt:
                page_num = h.get("page_index", -1) + 1
                context_parts.append(f"[{h.get('doc_id','unknown')} p.{page_num}] {_truncate_preserving_clinical(excerpt)}")
...
def derive_doc_id_from_row(row_in: Dict[str, Any]) -> Optional[str]:
    """
    Prefer explicit doc_id; else derive from case_id -> case_dir.name.
    Falls back to None if nothing resolvable.
    """
