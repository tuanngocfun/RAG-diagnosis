...
Gemini 2.5 Pro version — loads encoder/reranker once, uses Gemini for answering.
"""
...
from rag.reranking.gemini25pro_qdrant_bge import (
    CFG, qdrant, CQ2, Gemini25, TextCrossReranker, 
    rerank_with_text, find_case_dir, page_indices_to_paths,
    _qdrant_search, extractive_spans
)
...
def _read_existing_out(path: Path) -> Tuple[Set[str], Set[str]]:
    """Returns (done_keys, error_keys) from an append-only NDJSON output file."""
    done, err = set(), set()
    if not path.exists():
        return done, err
...
class BatchProcessor:
    ...
    def enhanced_qdrant_ask_text(self, question: str, doc_id: Optional[str] = None, 
        ...
        """Doc-scoped text retrieval using ColQwen2 text→image shared space."""
        qv = self.encoder.embed_texts([question])[0]

        musts = []
        # infer cutaneous from question if not provided (parity with module)
        if case_type is None:
            import re as _re
            if _re.search(r"\bcutaneous\b", question, _re.I):
                case_type = "cutaneous"
        if doc_id:
        ...
        # module-compatible any_keywords across keywords + entities with hyphen normalization
        if any_keywords:
        ...
        if micrograph_only and micrograph_strict:
            ...
            # parity fallback: if asked to prefer micrographs but none retrieved, try a strict pass
            if micrograph_only and not any((getattr(h, "payload", {}) or {}).get("micrograph_like") for h in hits):
                strict_filter = Filter(must=musts + [FieldCondition(key="micrograph_like", match=MatchValue(value=True))],
                                       should=should)
                hits2 = _qdrant_search(self.client, qv, pool_size, strict_filter, CFG.SCORE_THRESHOLD)
                if not hits2 and should:
                    strict_filter = Filter(must=musts + [FieldCondition(key="micrograph_like", match=MatchValue(value=True))])
                    hits2 = _qdrant_search(self.client, qv, pool_size, strict_filter, CFG.SCORE_THRESHOLD)
                if hits2:
                    hits = hits2

        # Normalize to dict hits
        ...
        # Soft preference for micrographs
    ...
    def answer_with_images(self, question: str, image_paths: List[str], hits: List[Dict[str, Any]]) -> str:
        """Answer using Gemini, adding cheap extractive text spans as Evidence."""
        paths = [Path(p) for p in image_paths if Path(p).exists()]
        if not paths:
            return "No valid images provided."
        spans = extractive_spans(hits, per_doc=2)
        # compact context to help site-aware answers
        ctx = " ".join([(h.get("text_excerpt") or "").strip() for h in hits[:6] if h.get("text_excerpt")])[:4000]
        return self.gem.answer(question, paths, spans=spans, context_text=ctx,
