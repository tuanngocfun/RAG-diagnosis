...
def generate_q1_symptom_only(case: Dict) -> Query:
    ...
    DEMOGRAPHIC BIAS FIX (per GPT 5.2): Put symptoms FIRST to prioritize
    symptom matching over demographic matching in BM25/E5 retrieval.
    """
    demographics = extract_demographics(case)
    symptoms = extract_symptoms(case.get("case_text", ""))
    
    # Symptoms first, demographics at end (per GPT 5.2 feedback)
    if symptoms:
        query_text = f"Patient presenting with {symptoms} ({demographics})"
    else:
        query_text = demographics
...
def generate_q3_image_only(case: Dict, images_dir: Path) -> Optional[Query]:
    """
    Q3: Image-only query
    Uses image caption from metadata instead of null text (per GPT 5.2/user feedback).
    """
    ...
    # Get image caption and type from metadata
    caption = first_image.get("caption", "")
    image_type = first_image.get("image_type", "medical_photograph")
    ...
    # Build text from image metadata (NO MORE null text!)
    demographics = extract_demographics(case)
    if caption:
        text = f"{demographics}. Clinical image: {caption}"
    else:
        # Fallback: use image_type description
        text = f"{demographics}. {image_type.replace('_', ' ').title()} image for diagnosis."
    
    return Query(
        case_id=case_id,
        query_type="Q3_image_only",
        text=text,  # Now has metadata-based text
...
class GroundTruth:
    ...
    diagnosis: str
    diagnosis_type: str  # CL, VL, MCL, PKDL, Other
    species: str = ""
...
def extract_ground_truth(case: Dict) -> GroundTruth:
    ...
    - diagnosis_type: CL, VL, MCL, PKDL, Cutaneous, Visceral, etc.
...
def generate_standardized_q1(case: Dict) -> StandardizedEvalQuery:
    ...
    # This is the CONTEXT, not the query (symptoms first per GPT 5.2)
    if symptoms:
        clinical_context = f"Patient presenting with {symptoms} ({demographics})"
    else:
        clinical_context = demographics
...
def generate_standardized_q3(case: Dict, images_dir: Path) -> Optional[StandardizedEvalQuery]:
    ...
    Uses image caption from metadata instead of null context (per user feedback).
    """
    images = case.get("images", [])
    if not images:
        return None
    ...
    # Build clinical context from image metadata (NO MORE null!)
    demographics = extract_demographics(case)
    if caption:
        clinical_context = f"{demographics}. Clinical image: {caption}"
    else:
        clinical_context = f"{demographics}. {image_type.replace('_', ' ').title()} image."
    
    return StandardizedEvalQuery(
        case_id=case_id,
        query_type="Q3_image_diagnosis",
        question=std_query.question_only,
        clinical_context=clinical_context,  # Now uses image metadata
