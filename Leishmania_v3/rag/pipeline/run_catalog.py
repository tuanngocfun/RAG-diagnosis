...
def create_run_metadata(run_dir: Path) -> RunMetadata:
    ...
    # Data fingerprints
    train_file = "train_clean.jsonl" if (DATA_ROOT / "train_clean.jsonl").exists() else "train.jsonl"
    test_file = "test_clean.jsonl" if (DATA_ROOT / "test_clean.jsonl").exists() else "test.jsonl"
    ...
    return RunMetadata(
        ...
        answers_hash=get_file_hash(run_dir / "answers.jsonl"),
        ragas_hash=get_file_hash(run_dir / "ragas.jsonl"),
        git_commit=get_git_commit(),
        metrics=metrics,
        # Data fingerprints
        train_file=train_file,
        train_hash=get_file_hash(DATA_ROOT / train_file),
        test_file=test_file,
        test_hash=get_file_hash(DATA_ROOT / test_file),
