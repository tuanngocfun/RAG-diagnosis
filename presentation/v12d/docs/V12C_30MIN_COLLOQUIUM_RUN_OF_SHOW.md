# V12c 30-Minute Colloquium Run Of Show
...
- Slides 20-25: appendix-only evidence for supervisor questions.
...
input-output-support path. The case IDs were verified absent from the official
121-case experimental retrieval corpus; the locked live-demo outputs used a
smaller local defense demo KB."
...
- Fallback line: "I will use the locked live-demo recapture here because the
  scientific point is the traceable input-output-evidence path, not the latency
  of this particular live execution."
...
- Provenance line: "These examples come from the 56-case held-out evaluation
  set and were checked as absent from the official 121-case experimental
  retrieval corpus. Their locked live-demo outputs used the local defense demo
  KB."
- Label line: "The targets are silver references from the thesis pipeline, not
  clinician-adjudicated ground truth; case 3 is explicitly label-conflicted
  because the verified reference is Non-Leish while the pseudolabel and live
  evidence are CL/leishmaniasis-plausible."
...
case came from the held-out evaluation set, compare the compact slide summary
against the original evaluation JSON, or ask whether RAG changed the selected
case-level outputs. Do not present three cases as a replacement for the thesis
